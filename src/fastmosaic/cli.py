...
def main():
    ...
    # -------- preprocess --------
    p_pre = subparsers.add_parser("preprocess", help="Prepare dataset")
    ...
    # -------- run-gps --------
    p_gps = subparsers.add_parser("run-gps", help="GPS-guided stitching")
    p_gps.add_argument("--frames",    default="data/raw/gps_mission/frames")
    p_gps.add_argument("--waypoints", default="data/raw/gps_mission/waypoints")
    p_gps.add_argument("--csv",       default=None)
    p_gps.add_argument("--out",       default="data/outputs/map_gps.png")
    p_gps.add_argument("--res",       type=float, default=0.05)
    p_gps.add_argument("--fov",       type=float, default=80.0)
    p_gps.add_argument("--no-refine", action="store_true")

    # -------- realtime --------
    p_rt = subparsers.add_parser("realtime", help="Real-time RTSP mapping")
    ...
    if args.command == "run":
        ...
        print(f"  Loaded {len(metas)} frames, starting ORB stitch...")
        cfg = StitchConfig(orb_features=6000, ratio_test=0.70)
        stitch_sequence(metas, args.out, cfg)
        print("✓ ORB stitching done  →", args.out)
        return
    ...
    if args.command == "run-sift":
        ...
        if len(metas) < 2:
            raise SystemExit("Need at least 2 images.")
        print(f"  Loaded {len(metas)} frames, starting SIFT stitch...")
